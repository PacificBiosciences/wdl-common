...
task cpg_pileup {
  ...
  command <<<
    ...
    for prefix in ~{out_prefix}.hap1 ~{out_prefix}.hap2 ~{out_prefix}.combined; do
      [ -f "$prefix".bw ] && mv --verbose "$prefix".bw "$prefix".cpg_pileup.bw
      [ -f "$prefix".bed ] \
        && mv --verbose "$prefix".bed "$prefix".cpg_pileup.bed \
        && bgzip "$prefix".cpg_pileup.bed \
        && tabix --preset bed "$prefix".cpg_pileup.bed.gz
    done
  >>>
