...
task sawfish_discover {
  ...
  command <<<
    ...
    echo ~{if defined(sex) then "" else "Sex is not defined for ~{out_prefix}.  Defaulting to karyotype XX for HiFiCNV."}
  ...
  runtime {
    docker: "~{runtime_attributes.container_registry}/sawfish@sha256:db7d78f8e5b6e607cfdcf25c21cb244dfddf5a41a5cdfd1302ab7c7646a2abe1"
    cpu: threads
    memory: mem_gb + " GB"
    disk: disk_size + " GB"
...
task sawfish_call {
  ...
  command <<<
    ...
    mv ~{out_prefix}/supporting_reads.json ~{out_prefix}.supporting_reads.json
  >>>

  output {
    File vcf              = "~{out_prefix}.vcf.gz"
    File vcf_index        = "~{out_prefix}.vcf.gz.tbi"
    File supporting_reads = "~{out_prefix}.supporting_reads.json"
  }
